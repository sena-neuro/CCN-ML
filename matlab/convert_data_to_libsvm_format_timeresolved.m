...
function [] = convert_data_to_libsvm_format_timeresolved(timeWindow)
...
% TimePoints: 
% 1:400 array => 1:100 indices - baseline, 101-400 indices - the video period
%                To take every 100 ms, you should take 50-element vectors,
%                e.g. the first 100 ms corresponds to 101-150, 
%                     100-200 ms corresponds to 151-200 indices
%                     200-300 ms corresponds to 201-250 indices
%                     300-400 ms corresponds to 251-300 indices
%                     400-500 ms corresponds to 301-350 indices
%                     500-600 ms corresponds to 351-400 indices

%                     -200 - -100 ms corresponds to 1-50 indices
%                     -100 - 0 ms corresponds to 51-100 indices


%% Prepare data for classification by using the whole time series - not time-resolved

for i = 1:size(video_robot,3) % ROBOT
    temp = video_robot(:,timeWindow,i); % take only a specific time window
    temp2 = temp(:);
    trials_robot(i,:) = temp2; % NoOfTrials x AllChannelsAndTimePoints_concatanated
end

for i = 1:size(video_android,3) % ANDROID
    temp = video_android(:,timeWindow,i); % take only a specific time window
    temp2 = temp(:);
    trials_android(i,:) = temp2; % NoOfTrials x AllChannelsAndTimePoints_concatanated
end

for i = 1:size(video_human,3) % HUMAN
    temp = video_human(:,timeWindow,i); % take only a specific time window
    temp2 = temp(:);
