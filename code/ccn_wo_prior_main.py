...
def task(args, dataFileList, time_ranges,subjectNo):
    subjFileList = dataFileList[subjectNo]
    acc = generalized_pipeline(subjFileList, start=0, end=400, window_size=args.w_size,
                               window_shift=args.shift,
                               gridsearch=args.gridsearch, verbose=args.verbose, time_ranges=time_ranges)

    results_dict[str(subjectNo)] = acc
    print(acc)
    plt.plot(time_ranges, acc)
    plt.savefig(args.save_path + 's' + str(subjectNo) + '_accuracy.png',
                bbox_inches='tight')
    plt.clf()
    plt.close()
    return acc
...
def generalized_pipeline(subjFileList, start=0, end=400, method='svc', gridsearch=True, window_size=100,
                         ...
                         verbose=True, time_ranges=None, plot=False):
    # check for different separate time ranges
    accuracies = []
    for time in range(start, end, window_shift):
        if not (time + window_size > end): # TODO think about changing here
            x, y = ccn_algorithms.create_windowed_data(subjFileList, 0, #TODO use all channels with an argument
...
def main():
    ...
    dataFileList = []
    subjFileList = []
    time_ranges = list(range(0, 400, args.shift))
    ...
    pool = Pool(processes=len(dataFileList))
    subjectNos = [i for i in range(len(dataFileList))]
    results = pool.starmap(task, zip(itertools.repeat(args),itertools.repeat(dataFileList),itertools.repeat(time_ranges), subjectNos))
    pool.close()
    pool.join()
    avg_accuracies = np.mean(np.array(results), axis=0)
    results_dict['all'] = avg_accuracies
